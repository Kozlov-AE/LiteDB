...
namespace LiteDB_V6
{
    ...
    internal class FileDiskService : IDisposable
    {
        ...
        public FileDiskService(Stream stream, string password)
        {
            ...
            if (password != null)
            {
                _crypto = new LiteDBv4.AesEncryption(password, SALT);
                _password = LiteDBv4.AesEncryption.HashSHA1(password);
            }
        }
        ...
        public virtual byte[] ReadPage(uint pageID)
        {
            ...
            if (pageID == 0 && _crypto != null)
            {
                ...
                if (LiteDBv4.BinaryExtensions.BinaryCompareTo(_password, header.Password) != 0)
                {
                    throw LiteDBv4.LiteException.DatabaseWrongPassword();
                }
            }
